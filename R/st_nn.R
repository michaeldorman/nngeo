...
st_nn = function(x, y, sparse = TRUE, k = 1, maxdist = Inf, returnDist = FALSE) {
  ...
  if(!class(st_geometry(x))[1] == "sfc_POINT" | !class(st_geometry(y))[1] == "sfc_POINT") {
    result = .st_nn_pol(x, y, k, maxdist)
  } else {
    if(st_is_longlat(x) & st_is_longlat(y)) {
